...
def init():
  ...
  def generatRecord(channel):
    record = {}
    record["biliId"] = channel.get("tid", 0) or 0
    record["name"] = channel.get("name", "")
    record["route"] = channel.get("route", "")
    record["desc"] = channel.get("desc", "")
    return record

  def getInsertSQLByRecord(record):
    print(record)
    insertSQL = "insert into Channel (name, biliId, grade, route, pId, channel_desc) values ('%(name)s', %(biliId)d, %(grade)d, '%(route)s', %(pId)d, '%(desc)s');" % record
    print('insertSQL', insertSQL)
    return insertSQL

  with UsingMysql(log_time=True) as um:
    for mainChannel in channels:
      grade1Record = generatRecord(mainChannel)
      grade1Record["grade"] = 1
      grade1Record["pId"] = 0
      subs = mainChannel.get("sub", [])
      insertSQL = getInsertSQLByRecord(grade1Record)
      um._cursor.execute(insertSQL)
      for sub in subs:
        grade2Record = generatRecord(sub)
        grade2Record["grade"] = 2
        grade2Record["route"] = '/v/' + grade1Record.get('route', '') + '/' + grade2Record.get('route', '')
        grade2Record["pId"] = grade1Record.get('biliId', 0)
        insertSQL = getInsertSQLByRecord(grade2Record)
        um._cursor.execute(insertSQL)
